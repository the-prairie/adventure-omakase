...
(() => {
  ...
  window.OmakaseOutings = function ({
    ...
    const button = (label, action, id = '', cls = 'subtle', extra = '') =>
      `<button type="button" class="${cls.split(/\s+/).includes('text-btn') ? cls : `btn ${cls}`}" data-outing="${action}" data-stop-id="${E(id)}" ${extra}>${label}</button>`;
