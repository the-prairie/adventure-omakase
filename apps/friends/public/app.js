...
(() => {
  ...
  function experienceContext(a) {
    ...
    return `<section class="experience-context"><p class="lede">${E(a.why)}</p><details class="form-detail"><summary>Experience guide & sources</summary>${guide?.summary ? `<p>${E(guide.summary)}</p>` : ''}${guide?.highlights?.length ? `<ul>${guide.highlights.map((line) => `<li>${E(line)}</li>`).join('')}</ul>` : ''}${guide?.planning ? `<p>${E(guide.planning)}</p>` : ''}${safeURL(guide?.source || a.source) ? `<a class="text-btn" href="${E(safeURL(guide?.source || a.source))}" target="_blank" rel="noopener noreferrer">${E(guide?.sourceLabel || 'Research source')} ${I('external')}</a>` : ''}${guide?.readAt ? `<p class="small muted">Source read ${dateText(guide.readAt)}. Recheck opening and bookings before going.</p>` : ''}</details></section>`;
  }
