...
(() => {
  ...
  const btn = (label, action, id = '', cls = '') =>
    `<button type="button" class="${cls.split(/\s+/).includes('text-btn') ? cls : `btn ${cls}`}" data-action="${action}" ${id ? `data-id="${E(id)}"` : ''}>${label}</button>`;
  ...
  function openDayIdea() {
    const region =
      S.me.profile?.windows?.find(
        (w) => w.from && w.to && w.from <= ui.day && ui.day <= w.to,
      )?.region || ui.homeRegion;
    const id = window.OMAKASE.homeMenu?.[region]?.[0];
    const c = window.OMAKASE.collections.find((c) => c.id === id);
    return c
      ? `<article class="open-day-idea"><p class="small muted">An idea for ${E(R[region])} · choose it if the area suits</p><h3>${E(c.title)}</h3><p>${E(collectionDuration(c))}</p>${btn('Explore this outing ' + I('arrow'), 'collection-story', c.id, 'subtle')}</article>`
      : '';
  }
  function compactDay() {
    const items = S.plans
    ...
    const rows = items
      .map((item) => {
        ...
        return `<article class="commitment agenda-entry ${p.status === 'cancelled' ? 'cancelled' : ''}"><button class="agenda-row" data-action="plan-detail" data-id="${E(p.id)}"><time>${missing ? 'Review' : `<strong>${E((part || p).start)}</strong><span>–${E((part || p).end)} JST</span>`}</time><span class="agenda-copy"><strong class="invite-title">${E(p.title)}</strong><span class="agenda-place">${E(missing ? 'Choose a new meeting option' : part?.meeting || p.area)}</span><span class="agenda-participation ${needs || missing ? 'needs-review' : ''}">${E(item.status)}</span></span>${image ? `<img src="${E(image.smallPath || image.path)}" width="64" height="64" loading="lazy" alt="${E(image.caption)}; photo credits below">` : `<span class="agenda-row-icon" aria-hidden="true">${I('arrow')}</span>`}</button></article>`;
      })
      .join('');
    const credits = [
      ...new Map(
  ...
  function collectionDuration(c) {
    if (c.travelScale === 'separate-stay') return c.duration;
    const model = window.OmakaseOutingModel;
    const result = model.evaluate(model.createDraft(c, C, ui.day), S.trip);
    return `${model.duration(result.totalMinutes)} · ${c.travelScale === 'day-trip' ? 'at stops; outward / return travel extra' : 'including time between stops'}`;
  }
  function homeMenu() {
    const ids = window.OMAKASE.homeMenu?.[ui.homeRegion] || [];
    return ids
      .map((id) => window.OMAKASE.collections.find((c) => c.id === id))
      .filter(Boolean);
  }
  ...
  function homePool() {
    return C.filter(
      (a) =>
        ...
        a.area === homeArea() &&
        a.minutes <= ui.homeMinutes &&
        (ui.homeMood === 'all' || a.mood === ui.homeMood) &&
        !/[bodw]/.test(a.flags) &&
  ...
  function diceInvitation() {
    const photos = homePool()
      .filter((a) => a.photo)
      .slice(0, 3);
    return `<button class="chance-invitation" data-action="home-roll" aria-label="Pick up the dice">${photos.map((a) => `<img src="${E(a.photo.smallPath || a.photo.path)}" alt="" loading="lazy" width="160" height="200">`).join('')}<span class="chance-invitation-die" aria-hidden="true">${I('dice')}</span></button>`;
  }
  ...
  function overlapDay() {
    ...
    return `<section class="overlap-day" aria-label="Shared areas and invitations for the selected day">${
      Object.keys(R)
        .filter(
          (region) =>
            rows.some((r) => r.window.region === region) ||
            S.plans.some(
              (p) =>
  ...
  function showDiscovery(id) {
    ...
    openModal(
      ...
      <div class="place-personal-actions">${btn(I('people') + (pick?.shared ? 'Stop sharing this pick' : 'Recommend to the group'), 'recommend', a.id, 'subtle')}${pick ? btn('View saved ideas ' + I('arrow'), 'view-saved', '', 'text-btn') : ''}${mode === 'shared' ? btn('Check this for my dates', 'ask-check', a.id, 'text-btn') : ''}</div><p class="small muted place-private-note">${pick?.shared ? 'Recommended with your name to this private trip.' : 'Saves are private. Recommendations are visible to your trip.'}</p>
      ...
      `<h2>Trip settings</h2><section class="settings-row"><h3>My name & travel dates</h3><p>Help friends know where your paths might cross.</p>${btn('Edit my details', 'profile', '', 'subtle')}</section>${S.me.role === 'owner' && mode === 'shared' ? `<section class="settings-row"><h3>Our trip window</h3><p>${dateText(S.trip.start)} — ${dateText(S.trip.end)}. Everyone still chooses their own travel dates.</p>${btn('Edit trip window', 'trip-window', '', 'subtle')}</section>` : ''}<section class="settings-row"><h3>Bring someone along</h3><p>One reusable link for friends. No email or password.</p>${btn('Get the trip link', 'invite', '', 'primary')}</section>${mode === 'shared' ? `<section class="settings-row"><h3>Another device?</h3><p>Continue with the same name, saved ideas and plans.</p>${btn('Make my device link', 'device-link', '', 'subtle')}</section>` : ''}<section class="settings-row"><h3>Bring my original shortlist</h3><p>Import saved ideas from the old fieldbook. Old notes stay in that book; they will not suddenly appear here.</p><label class="btn" for="legacy-import">Import old shortlist</label><input id="legacy-import" type="file" accept="application/json,.json" hidden><p id="import-status" role="status"></p></section><section class="settings-row"><h3>A copy of our plans</h3><p>A readable JSON snapshot of your current view. Photo files are separate; the operator backup saves both.</p>${btn('Download snapshot', 'export', '', 'subtle')}</section>${removed.length ? `<section class="settings-row"><h3>Changed your mind?</h3><p>Restore a memory you removed in the last seven days.</p>${removed.map((m) => `<div class="row between"><span>${E(m.title || 'A memory')}</span>${btn('Restore', 'moment-restore', m.id, 'subtle')}</div>`).join('')}</section>` : ''}${
        S.me.role === 'owner' && mode === 'shared'
          ? `<details class="settings-row"><summary>Occasional owner controls</summary><p>You manage access, not everyone’s day.</p>${S.members
              ...
              .join('')}</details>`
          : ''
      }<details class="settings-row"><summary>Research, connection & artwork</summary><p>The original 300 entries remain research leads with their existing source labels, not live availability. Photographs carry their original source, date and license. Decorative illustrations are labelled separately. Neither proves current conditions.</p><p>The app checks for changes about every 20 seconds while visible, and again when you return. It does not run that poll in the background. In-app updates only; there are no push notifications. Joining requires a connection.</p></details><section class="settings-row">${mode === 'demo' ? btn('Reset example trip', 'reset-demo', '', 'subtle') : btn('Sign out on this device', 'logout', '', 'subtle')}</section>`,
      'settings',
    );
  }
  ...
  async function handleClick(e) {
    ...
    try {
      switch (action) {
        ...
        case 'home-undo':
          ui.homeHistory.pop();
          ui.homePickId = ui.homeHistory.at(-1) || null;
          render();
          app.querySelector('#home-dice-result')?.focus();
          break;
        case 'home-roll':
          await rollHomeDice();
          break;
  ...
  app.addEventListener('change', (event) => {
    if (event.target.id === 'friends-day') {
      ui.day = event.target.value;
      render();
      app.querySelector('#friends-day')?.focus({ preventScroll: true });
      return;
    }
    if (!['home-time', 'home-mood'].includes(event.target.id)) return;
    const id = event.target.id;
    if (id === 'home-time') ui.homeMinutes = Number(event.target.value);
    else ui.homeMood = event.target.value;
    ui.homePickId = null;
    ui.homeHistory = [];
    render();
    app.querySelector('#' + id)?.focus({ preventScroll: true });
  });
  ...
  const outings = window.OmakaseOutings({
    ...
    icon: I,
    openModal,
    publishInvitation: async (payload, panel, scope) => {
      if (scope !== `${mode}-${S.trip.id}-${S.me.id}`)
        throw Error('Your session changed. Reopen this outing.');
      const result = await api('/plans', 'POST', payload);
      outings.clear(payload._collectionId);
      await refresh();
      if (panel.isConnected && dialog.open) showPlan(result.id);
      toast('Your invitation is open. Friends can join any available part.');
    },
    toast,
  });
