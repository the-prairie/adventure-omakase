...
(() => {
  ...
  function render() {
    ...
    app.innerHTML =
      header() +
      `<div class="container"><div class="meta-line"><span>${E(S.trip.name)} <span class="muted">/ ${E(S.me.name)}’s view</span></span><span class="row"><span class="meta-date">${dateText(S.trip.start)} — ${dateText(S.trip.end)}</span><span>${mode === 'demo' ? 'Local example' : `<i class="live-dot"></i>${online ? 'Shared trip' : 'Offline copy'}`} · JST</span></span></div>${mode === 'shared' ? '<div class="companion-bar"><button class="text-btn" data-action="companion">✳ Travel companion</button><span class="small muted">Help along the way</span></div>' : ''}<main id="main" tabindex="-1">${({ plans: board, day: myDay, discover: discover, people: peoplePage, story: storyPage }[ui.view] || board)()}</main>${footer()}</div><nav class="nav-dock" aria-label="Quick navigation">${navButtons(true)}</nav>`;
    if (ui.view === 'day')
  ...
  function board() {
    ...
    return `<section class="trip-home"><div><div class="eyebrow">Together, apart</div><h1>${E(S.me.name)}’s trip.</h1><p>Good company, when you want it.</p>${btn('Find an idea ' + I('arrow'), 'discover-nav', '', 'primary')}</div><div class="next-plan"><div class="eyebrow">${next ? (changed ? 'Your next plan · review changes' : 'Your next commitment') : 'Your plans start here'}</div><h2>${next ? E(next.p.title) : 'No upcoming commitments.'}</h2><p>${next ? `${dateText(next.p.date)} · ${formatTime(next.part)} JST${next.part !== next.p ? ' · ' + E(next.part.label) : ''}` : 'Join a friend below, or put your own idea out there. Your calendar stays yours.'}</p>${next ? `<p class="small">${E(next.part.meeting)}</p>${btn(changed ? 'Review and reconfirm' : 'See my plan ' + I('arrow'), 'plan-detail', next.p.id, 'subtle')}` : btn('Start an invitation ' + I('plus'), 'plan-new', '', 'subtle')}</div></section><section><div class="section-heading"><div><div class="eyebrow">The invitation board</div><h2>Who’s up for what?</h2><p>Open plans, not obligations. Everything here is optional.</p></div><button class="text-btn" data-action="plan-new">Start something ${I('plus')}</button></div><div class="body-split"><div>${regionFilters('board')}<div class="invitation-list">${plans.length ? plans.map(invitation).join('') : empty('The day is still yours.', 'No open plans in this view. Start one, widen your filters, or leave the day unplanned.', 'Open a plan', 'plan-new')}</div>${
    ...
    try {
      switch (action) {
        ...
        case 'companion':
          openModal(
            'Travel companion',
            `<h2>What would help?</h2><p class="lede">A useful hand while you’re out.</p><div class="companion-options"><button data-action="ask-find"><strong>Find an idea</strong><span>Research something for your dates</span></button><button data-travel="open" data-value="places"><strong>Nearby places</strong><span>Look for a place around you</span></button><button data-travel="open" data-value="route"><strong>Directions</strong><span>Check the way there</span></button><button data-travel="open" data-value="translate"><strong>Translate</strong><span>Understand a sign or a few words</span></button><button data-travel="open" data-value="memory"><strong>Tell a memory</strong><span>Turn a voice note into a story</span></button><button data-travel="open" data-value="search"><strong>Research</strong><span>Look up a specific question</span></button><button data-travel="watches"><strong>My checks</strong><span>See updates you asked to follow</span></button></div>`,
            'companion',
          );
          break;
        case 'people-nav':
