...
(() => {
  ...
  const formatTime = (p) => `${p.start}–${p.end}`;
  const photo = (id) =>
    mode === 'demo'
        ...
        : r?.status === 'interested'
          ...
          : r?.status === 'waitlist'
            ...
            : p.kind === 'idea'
              ? 'Who’s keen?'
              : 'I’m going';
    return `<article class="invitation ${p.status === 'cancelled' ? 'cancelled' : ''} ${needs ? 'changed' : ''}" data-plan-card="${E(p.id)}"><div class="date-column"><small>${dateText(p.date, { month: 'short' })}</small><b>${p.date.slice(-2)}</b><small>${dateText(p.date, { weekday: 'short' })}</small><span class="vertical">${p.region === 'osaka' ? 'Osaka' : R[p.region]}</span></div><div class="invitation-content">${needs ? '<div class="tiny-banner">Details changed since you joined. Take another look.</div>' : ''}<div class="row invitation-header">${avatar(p.hostId)}<div class="host-name grow">${E(host.name)} <span>· ${mine ? 'hosting' : 'an open invitation'}</span></div><span class="pill ${p.status !== 'open' ? '' : needs || p.kind === 'idea' ? 'rust' : 'green'}">${p.status !== 'open' ? E(p.status) : tag}</span></div><button class="invite-title" data-action="plan-detail" data-id="${E(p.id)}">${E(p.title)}</button><p class="invite-description">${E(p.description.length > 185 ? p.description.slice(0, 182) + '…' : p.description)}</p><div class="invite-meta"><span>${I('clock')}${formatTime(p)} JST</span><span>${I('pin')}${E(p.area)}</span>${p.capacity ? `<span>${I('people')}${1 + members.length}/${p.capacity} places incl. host</span>` : ''}</div><div class="invite-footer"><div><div class="row" style="gap:8px"><span class="avatar-stack">${avatar(p.hostId)}${members
      .slice(0, 3)
  ...
  function discoveryCard(a) {
    ...
    return `<article class="discovery ${a.featured ? 'featured' : ''}"><div class="row between"><span class="number">${a.custom ? 'FRIEND’S FIND' : a.region.toUpperCase().slice(0, 3) + ' / ' + String(a.n).padStart(3, '0')}</span><span class="pill">${I(MOODS[a.mood])}${E(a.mood)}</span></div><h3><button data-action="discovery" data-id="${a.id}">${E(a.title)}</button></h3><p>${E(a.why)}</p>${a.custom ? `<p class="find-author">Found by ${E(person(a.memberId).name)}</p>` : ''}<div class="bottom"><span>${E(a.area)}<br>${a.minutes < 60 ? a.minutes + ' min' : (a.minutes / 60).toFixed(a.minutes % 60 ? 1 : 0) + ' hr'} on site · estimate${friends.length ? `<br>${friends.map((f) => E(person(f.memberId).name)).join(', ')} saved this` : ''}</span><button class="icon-btn ${pick ? 'saved-star' : ''}" data-action="save" data-id="${a.id}" aria-label="${pick ? 'Unsave' : 'Save'} ${E(a.title)}" aria-pressed="${!!pick}">${I(pick ? 'check' : 'save')}</button></div></article>`;
  }
  function discover() {
  ...
  function calendarExport(plans, selectedChoice = null) {
    ...
    for (const p of plans) {
      ...
      lines.push(
        'BEGIN:VEVENT',
        `UID:${p.id}-${choice}@adventure-omakase`,
        `SEQUENCE:${p.revision}`,
        `DTSTAMP:${new Date()
          .toISOString()
          .replace(/[-:]/g, '')
