...
Keep the companion inside the same authenticated friends app. Gemini 3.8 Flash supplies structured suggestions and text/photo/audio interpretation when `GEMINI_API_KEY` is configured; the existing Workers AI adapter remains the fallback for ordinary Ask. Google Places API (New) and Routes API require `GOOGLE_MAPS_API_KEY`. Credentials remain Worker secrets. No external chat or map iframe replaces the app.
