...
Gemini tool continuations preserve the complete returned candidate content, including opaque thought signatures, and echo each function-call ID in its response. Multiple tool responses share one user turn. An Ask task allows at most two additional attempts for explicit HTTP 503 responses, within six total model attempts and three attempts per turn. Short Retry-After values (up to five seconds) are honored; otherwise exponential delay and jitter apply. Cancellation interrupts the delay. Every attempt gets its own budget reservation. Other errors and unknown transport outcomes are not retried. These protocol fixes do not by themselves prove the cause of historical 503 responses. The separate trip daily estimate ledger also releases the pending-call allowance for a typed definite rejection, while retaining measured costs from earlier successful turns. Unknown transport and cancellation outcomes retain conservative daily reservations. Historical entries are not reset automatically.
