...
At this checkpoint the ambitious dice redesign awaited the user's direction choice; the map/content implementation did not complete that request. Preview deployment of this implementation remained pending. Existing production and live-provider acceptance boundaries remained unchanged.
...
## September 8: tactile atlas dice

The user selected the atlas concept with tactile dice physics. Implementation `1dc5eee` adds drag/fling momentum, gravity and rebounds; a short map flight into the selected area; and a discovery card over the map. The existing filtered, unbiased draw without replacement is preserved. Keyboard/tap input, reduced motion, unlocated areas, tile failures and closing during animation remain usable. Attribution stays visible above the result card. The marker is an area reference, not a venue entrance. No new live provider call, booking, canonical plan or production mutation is introduced.

Final observed local gates: `pnpm check` passed all 12 tasks including 102 friends tests; `test:cloudflare` passed all 11 native workerd checks; `test:browser` passed all 42 Chromium/WebKit scenarios in five minutes. New coverage exercises actual pointer dragging, touchscreen tap emulation, camera landing, uncovered map attribution, keyboard/reduced-motion results, tile failure and cancellation. Desktop and phone-width screenshots were inspected. A local in-app browser also loaded real OSM tiles and completed a physical drag through to an actionable result. Physical-phone performance remains unverified.

The taller dialog exposed an existing choice-popup scroll dismissal: scrolling the trigger into view could immediately close the newly opened time menu. Highlighting now scrolls only the option list, and a visible popup follows its trigger when the dialog scrolls. The strengthened controls scenario passed in both browsers before the final full run. Earlier new-test setup failures used an incomplete area name and a zero-sized Leaflet pane visibility assertion; both test assumptions were corrected. An interrupted intermediate full run also recorded a Chromium `page.reload` connection refusal in the existing shared-trip scenario; the final run passed that scenario in both browsers without retries. Its transport cause is not established.

Local logs: `/tmp/omakase-atlas-browser-final.log`, `/tmp/omakase-atlas-gates-complete.log` and `/tmp/omakase-atlas-runtime.log`; synthetic screenshots remain under ignored `apps/friends/evidence/browser-artifacts`. Preview publication and exact-release verification are pending at this checkpoint. Earlier provider and physical-device gates are unchanged.

### Atlas preview verified

Preview release `113117ecb8c5a45e17627ccd1e0f27a219de3dc6` is exact-verified at September 8 04:46:59 UTC: Worker health, manifest and 19 assets match the local build byte-for-byte. The hosted in-app browser loaded 15/15 initial OSM tiles, accepted an actual drag and revealed the Nakanoshima-to-Kitahama walk after the map flight. Its source photo and ordinary detail/invitation actions were present. The local proof is `apps/friends/evidence/atlas-dice/preview-release.json`.

[Deployment run 34187494804](https://github.com/the-prairie/adventure-omakase/actions/runs/34187494804) succeeded, including 102 friends tests, 11 workerd checks, all 42 browser scenarios and the immediate HTTPS release gate. Worker version: `af4293f9-097f-4023-ae1f-7cc947a54c8f`. Main CI and Security also passed.

The separate [friends CI run 34187494803](https://github.com/the-prairie/adventure-omakase/actions/runs/34187494803) passed 40/42: one WebKit fixture could not start workerd because its selected port was already in use; another expected a map tile error after reopening an already visited area but received no error message. That test now selects an unvisited region and asserts requests were actually aborted, avoiding an assumption about cached tiles. The strengthened case passed in both local browsers (2/2). The port collision remains a distinct test-infrastructure failure; no blanket retry or test suppression was added. This final test/documentation follow-up changes no application asset. No merge or production deployment occurred.

## September 8: collapse preferences after a roll

The result now hides the region, area, mood, time, advanced options and shortlist controls. Roll again retains the same settings. Change preferences restores the form and focuses the visible region dropdown; a successful subsequent roll collapses it again. Reopening preserves values and empty-shortlist recovery remains available.

Observed locally: `pnpm check` passed all 12 tasks; the targeted dice-atlas and interactive browser suites passed all 6 Chromium/WebKit cases, including mobile width, retained choices, re-roll, keyboard focus, cancellation and empty-filter recovery. Desktop and mobile result screenshots were inspected. An initial run caught focus targeting the hidden native select; the final implementation focuses the visible custom control. Preview publication and exact release verification are pending at this checkpoint.

Release verification: implementation `f7c803f2b63ce0cba84275dd79c7e292750e5878` is live on the isolated preview. At 2026-09-08 06:21:27 UTC, Worker release, manifest and 19 asset bodies matched the local build. Hosted browser rolling, collapsed controls, restored values/focus and a subsequent roll were exercised. Main CI, Security and friends CI passed; friends CI observed 102 tests and 42 browser cases. Deployment run 34193549126 passed its tests and uploaded Worker `c2cb29f8-038a-4461-b48f-4e8e44574e30`, but failed its immediate post-upload asset/Worker consistency check. The later exact verification passed; the failed workflow is not described as green. No production deployment or physical-device acceptance is claimed.

## September 8: connected Google Map and Fieldbook

Map and Fieldbook now share the curated collection, region/area/search/mood/time filters, private saves and selected discovery. The explicit view choice persists on the device. Show on map focuses a discovery; source-backed site markers open its card, while area markers filter the list. Google navigation remains an external action. The compact mobile map view keeps the map closer to its controls.

Google Maps JavaScript loads only in Map view and reuses one map instance across ordinary renders and view switches. The new public configuration endpoint returns only the separately restricted browser key. The preview key is restricted to the preview origin and Maps JavaScript API. Failed loading or late authorization failure retains the Leaflet area fallback and Fieldbook. Fifteen researched site references supplement the existing 144 broad area anchors; this does not establish entrance, hours or complete venue coverage.

Observed local gates: `pnpm check` passed all 12 tasks (103 friends tests); workerd checks passed 11/11. The full browser suite passed 46/46 before the final compact-layout and late-authorization fallback refinement; the affected final suites passed 8/8 across Chromium and WebKit, including the new failure case. Mobile and desktop screenshots were inspected. Provider fixture results do not establish real Google rendering. Preview publication and hosted verification are pending at this checkpoint.

Release proof: implementation `187bd9c93ae182a480ceafe4993dd95f2186d4d0` is live on the isolated preview. At 2026-09-08 07:56:10 UTC, Worker health, release manifest and all 19 asset bodies matched the local build. Worker version: `bc55603b-42c0-4b51-ba26-994bf7674c41`. The actual Google map rendered tiles and attribution, accepted a Nezu Shrine marker click, opened its discovery, retained search/selection through Map/Fieldbook switches and reused one map instance with one script load. Desktop and phone-width screenshots were inspected; the hosted check observed no page errors or horizontal overflow. Evidence: `apps/friends/evidence/map-modes/`.

[Deployment run 34200931493](https://github.com/the-prairie/adventure-omakase/actions/runs/34200931493) passed 103 friends tests, 11 native checks, all 48 Chromium/WebKit cases and immediate HTTPS release verification. Main CI and Security passed. The separate [friends CI run 34200931383](https://github.com/the-prairie/adventure-omakase/actions/runs/34200931383) passed 47/48: its WebKit shared-journey test could not find the example page tagbar after navigation (`friends.spec.mjs:618`). The same case passed in the deployment run and targeted local reproduction (1/1, 19.8s); the one-off failure remains unexplained and is not reported as green. No test was disabled or given blanket retries. This evidence follow-up changes documentation only. No merge, production deployment or physical-phone acceptance is claimed.

## September 8: explain and recover unavailable dice

Reproduced the reported Osaka / Hyogo: Himeji state: its sole catalogue idea has the regional-excursion flag, so the default exclusion disabled the die while the explanation was below the atlas. Empty shortlists now explain the restriction at the die's position and provide Change filters plus an explicit Include these ideas and roll action when that setting is the blocker. The recovered Himeji draw, repeated roll and collapsed preferences work without silently broadening other filters.

Observed local gates: pnpm check passed all 12 tasks; all 8 affected Chromium/WebKit cases passed, including the new Himeji regression and existing fling, keyboard/reduced-motion, cancellation and empty-shortlist recovery. Desktop and phone-width screenshots were inspected; actual local browser controls recovered the Himeji result and Change filters focused the visible area chooser. Preview release proof is pending at this checkpoint.

Release proof: `a1dfa3901e7bcb25ac5feee31af10c4c5048d8a4` is live on preview, Worker `37620292-c5df-4220-92b0-2fdbf141fd0e`. At 2026-09-08 20:25:02 UTC, Worker, manifest and 19 assets matched the local build. The hosted browser reproduced the explanatory Himeji state, explicitly included its excluded idea, completed the Himeji almond-butter toast draw and completed Roll again. The result screenshot and exact-release proof are under `apps/friends/evidence/dice-empty/`.

[Deployment run 34273592223](https://github.com/the-prairie/adventure-omakase/actions/runs/34273592223) passed 103 friends tests, 11 native checks, all 50 browser cases and immediate release verification. Main CI and Security passed. Separate [friends CI 34273592253](https://github.com/the-prairie/adventure-omakase/actions/runs/34273592253) passed 49/50: the WebKit shared-journey test recorded an external Google Maps GetViewportInfo access-control error in its final console assertion (`friends.spec.mjs:622`). Both Himeji regressions passed; this external-iframe failure is separate from the dice repair and was not suppressed. This follow-up records evidence only. No merge or production deployment occurred.

## September 8: discovery content expansion

24 additional Osaka/Kansai and Okinawa guides now describe the actual visit, setting, possible activities and arrangements. 23 new licensed local photographs show the relevant place or a clearly captioned example. Coverage: 36 expanded guides; 50 photographed entries / 51 photographs; 300 total discoveries. All 24 sources were read on September 8; this is not a check of availability for the traveller’s dates. Photo subjects and provenance were inspected, including the Tatsuno example of almond toast and Hateruma’s daytime aerial view. Tower of the Sun links its official illustrated guide without a bundled photograph.
